...
class Analysis(object):
    ...
    #to extracts analysis results
    zc=[]
    eypy=[]
    psf=[]
    psft=[]
    ww=[]
    #to extract spha in longitudinal plot
    ...
    def spots(self, ax, heights=[1., .707, 0.],
        ...
        t = GeometricTrace(self.system)
        t.rays_point((0, hi), wavelengths[0], nrays=nrays,
                     distribution="hexapolar", clip=True)
        self.rd.append(t.rms())
        for axi in ax:
            for axii in axi:
                self.post_setup_axes(axii)

    def opds(self, ax, heights=[0., .707, 1.],
             wavelength=None, nrays=100, colors="grbcmyk"):
        paraxial = self.system.paraxial
    ...
    def longitudinal(self, ax, height=1.,
                     wavelengths=None, nrays=21, colors="rbcmykg"):
        ...
        for wwi in np.r_[wavelengths[0], ww]:
            ...
            y, u = self.system.aim((0, 0), (0, 1), pd, ph)

            t.rays_given(y, u, wwi)
            t.propagate(clip=False)
            print('after propagrating')
            print('t is')
            print(t.y)
            print(np.shape(t.y))
            print(t.y[-1,0,1])
    ...
    def return_spha(self,nrays=21,colors="rbcmykg"):
        wavelengths = self.system.wavelengths
        wl, wu = min(wavelengths), max(wavelengths)
        ww=np.linspace(wl,wu,nrays)
        z_spha=[]
